...
def get_data():
    try:
        ...
        label_file_name = 'labels600.txt'
        dictionary_file_name = 'dictionary600.pickle'
        ...
        detections = np.loadtxt('RCNN_output0.txt')
        for x in range(19):
            RCNN_output_file_name = 'RCNN_output' + str(x+1) + '.txt'
            detections = np.concatenate((detections, np.loadtxt(RCNN_output_file_name)))
        ...
        labels, dictionary = get_text_data_from_API()
...
def get_image_data_from_API(dataDir='.', input_height=512, input_width=512):
    """
    ...
    for i in range(20):
        ...
        RCNN_output_file_name = 'RCNN_output' + str(i) + '.txt'
        
        np.savetxt(RCNN_output_file_name, detections.reshape(detections.shape[0], -1))

def get_text_data_from_API(dataDir='.', num_images=600):
    """
	Given a file path to the downloaded COCO API folder, and the 
    number of images to load, return the processed images as a numpy
    array, and their corresponding captions as a list in their ids.
    ...
    labels, dictionary =  np.array(id_labels), index_dictionary
    print(labels.shape)
